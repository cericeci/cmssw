...
void PrimaryVertexProducerCUDA::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  reco::TransientTrack* gpu_inputTracks;
  cudaCheck(cudaMalloc(&gpu_inputTracks, sizeof(reco::TransientTrack)*t_tks.size()));
  ...
  cudaCheck(cudaMemcpy(gpu_inputTracks, &t_tks, sizeof(reco::TransientTrack)*t_tks.size(), cudaMemcpyHostToDevice));
  trackFilterCUDA::filterWrapper(gpu_inputTracks, gpu_outputBools.get(), t_tks.size());

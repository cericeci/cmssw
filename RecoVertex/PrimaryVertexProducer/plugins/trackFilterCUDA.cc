...
__global__ void trackFilterKernel(unsigned int ntracks, TrackForPV::TrackForPVSoA* tracks, filterParameters params, double* osumtkwt){
    ...
    for (unsigned int i = firstElement; i < tracks->stride(); i += gridSize) {
      ...
      if (tracks->isGood(i)){
        ...
        if (std::fabs(tracks->z(i)) > 1000.){ 
          ...
          tracks->dz2(i) = tracks->dz2(i) + (tracks->bx(i)*tracks->bx(i)*tracks->pxAtPCA(i)*tracks->pxAtPCA(i) + tracks->by(i)*tracks->by(i)*tracks->pyAtPCA(i)*tracks->pyAtPCA(i))* tracks->pzAtPCA(i)*tracks->pzAtPCA(i)/(( tracks->pxAtPCA(i)*tracks->pxAtPCA(i) + tracks->pyAtPCA(i)*tracks->pyAtPCA(i) )* ( tracks->pxAtPCA(i)*tracks->pxAtPCA(i) + tracks->pyAtPCA(i)*tracks->pyAtPCA(i))) + params.vertexSize*params.vertexSize; // TODO:: For sure ways to optimize this
          ...
          if (not(std::isfinite(tracks->dz2(i))) || tracks->dz2(i)< std::numeric_limits<double>::min()){ // Bad track dz2 is taken out
            ...
            if (params.d0CutOff > 0){ // Track weights are activated only if there is a non-zero cutoff
              ...
              if (not(std::isfinite(tracks->weight(i))) || tracks->weight(i)< std::numeric_limits<double>::min()){ // Bad track weight is taken out
                tracks->isGood(i) = false;
                tracks->weight(i) = 0;
                continue;
              }
    ...
    if (threadIdx.x == 0 && blockIdx.x==0){ // Single threaded. TODO:: Check atomicAdd or other sync ways of doing this
      ...
      for (unsigned int i = 0 ; i < ntracks ; i++){
        if (tracks->isGood(i)) (*osumtkwt) += tracks->weight(i);
      }
  ...
  void filterWrapper(unsigned int ntracks, TrackForPV::TrackForPVSoA* tracks, filterParameters params, double* osumtkwt, cudaStream_t stream){
    ...
    unsigned int gridSize  = (8096 + blockSize - 1)/blockSize; // TrackForPV::TrackForPVSoA always has 8096 entries. TODO::Automatize this one

...
__global__ void trackFilterKernel4D(unsigned int ntracks, TrackForPV::TrackForPVSoA* tracks, filterParameters params, double* osumtkwt){
    // TODO:: Study performance of explicitly writing squares vs std::pow. Study performance of atomics for the serial part
    size_t firstElement = threadIdx.x + blockIdx.x * blockDim.x;
    size_t gridSize = blockDim.x * gridDim.x;
    // First the preselection block
    ...
    for (unsigned int i = firstElement; i < tracks->stride(); i += gridSize) {
      ...
      if (tracks->isGood(i)){
        ...
        if (std::fabs(tracks->z(i)) > 1000.){ 
          ...
          tracks->dt2(i) = tracks->dt2(i)
                         + params.vertexSizeTime*params.vertexSizeTime;
          if (not(std::isfinite(tracks->dt2(i))) || tracks->dt2(i)< std::numeric_limits<double>::min()){
            tracks->isGood(i) = false;
            tracks->weight(i) = 0;
          }
          else{
...
void filterWrapper4D(unsigned int ntracks, TrackForPV::TrackForPVSoA* tracks, filterParameters params, double* osumtkwt, cudaStream_t stream){
    unsigned int blockSize = 512; // TODO::Optimize this
    unsigned int gridSize  = 1; // TrackForPV::TrackForPVSoA always has 8096 entries. TODO::Automatize this one
    trackFilterKernel4D<<<gridSize, blockSize, 0, stream>>>(ntracks, tracks, params, osumtkwt);
    cudaCheck(cudaGetLastError());
}

#include "L1Trigger/L1TMuonOverlapPhase1/plugins/MuonExtrapolator.h"

MuonExtrapolator::MuonExtrapolator(const edm::ParameterSet& iCfg) {
...
MuonExtrapolator::~MuonExtrapolator(){
}

void MuonExtrapolator::beginJob(){
}

void MuonExtrapolator::endJob(){
}

void MuonExtrapolator::produce(edm::Event& iEv, const edm::EventSetup& eventSetup){
...
float MuonExtrapolator::propagateGenPart(std::vector<reco::GenParticle>::const_iterator gP){
...
float MuonExtrapolator::propagateGenPartPhi(std::vector<reco::GenParticle>::const_iterator gP){
  int charge = gP->charge();
  GlobalPoint r3GV(gP->vx(), gP->vy(), gP->vz());
  GlobalVector p3GV(gP->px(), gP->py(), gP->pz());
  GlobalTrajectoryParameters tPars(r3GV, p3GV, charge, &*magField);
...
DEFINE_FWK_MODULE(MuonExtrapolator);

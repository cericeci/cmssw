...
#leg.AddEntry(picks["all"]["Disp_HighPt_PU200_DP_all"]['hEta20_q12_cut20'],"l","All")
...
for t in tags:
  if first:
    first = False
    thePass = picks[t]["Disp_HighPt_PU200_DP_%s"%t]['hEta20_q12_cut20'].GetPassedHistogram() 
    theTotal= picks[t]["Disp_HighPt_PU200_DP_%s"%t]['hEta20_q12_cut20'].GetTotalHistogram()
    toPlot[t] = ROOT.TEfficiency(thePass,theTotal)
    toPlot[t].SetLineColor(colors[t])
    toPlot[t].SetTitle(";Generated muon  #eta ;Efficiency")
    leg.AddEntry(toPlot[t],pretty[t],"l")
    toPlot[t].Draw()
    canvas9.Update()
    toPlot[t].GetPaintedGraph().SetMaximum(0.2)
  else:
    theAddPass = picks[t]["Disp_HighPt_PU200_DP_%s"%t]['hEta20_q12_cut20'].GetPassedHistogram()
...
leg.Draw("same")
canvas9.SaveAs('~/www/omtf_2020/Jul27/SingleMu/tdr_eta_patternsplit_DispHighPt.pdf')
canvas9.SaveAs('~/www/omtf_2020/Jul27/SingleMu/tdr_eta_patternsplit_DispHighPt.png')

...
SuepShower::SuepShower(double mass, double temperature, Pythia8::Rndm* rndmPtr) {
  ...
  p_plus_ = (boost::math::tools::bisect(boost::bind(&SuepShower::logTestFunction, this, boost::placeholders::_1), pmax, 50.0, tolerance_)).first;  // first root
  p_minus_ = (boost::math::tools::bisect(boost::bind(&SuepShower::logTestFunction, this, boost::placeholders::_1), 0.0, pmax, tolerance_)).first;  // second root
...
std::vector<Pythia8::Vec4> SuepShower::generateShower(double energy) {
  ...
  double scale;
  scale = (boost::math::tools::bisect(boost::bind(&SuepShower::reballanceFunction, this, boost::placeholders::_1, shower), 0.0, 2.0, tolerance_)).first;
...
const double SuepShower::fMaxwellBoltzmann(double p) { return p * p * exp(-mass_over_T_ * p * p / (1 + sqrt(1 + p * p))); }
...
const Pythia8::Vec4 SuepShower::generateFourVector() {
  ...
  Pythia8::Vec4 daughterFourMomentum = Pythia8::Vec4(momentum * cos(phi) * sin(theta), momentum * sin(phi) * sin(theta), momentum * sin(theta), en);
